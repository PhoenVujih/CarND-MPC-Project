 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (unsigned int i = 0; i < N; i++) {
      fg[0] += cte_coeff * CppAD::pow(vars[i + cte_start], 2);
    ...
    for (unsigned int i = 0; i < N - 2; i++) {
      fg[0] += delta_dot_coeff * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      fg[0] += a_dot_coeff * CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
    }
